 ...
 *
 *   1. ONE `full` shift: a closed shift of ten hours or more (see `workedTime`). One shift row,
 *      one driver, both slots.
 ...
 *
 * `pending` is not a hedge, it is the honest state. ANY running shift becomes a single or a double
 * depending on how long it runs — a morning start and an evening start alike can reach ten hours —
 * and `workedTime` deliberately reports `unknown` until the close. A badge that said «عادية» at
 * noon and «دبل» that night would be changing its story in front of the manager.
 *
 ...
 *
 * Pure and string-free, like the rest of the domain: it returns a code the screens translate.
 */

import type { ShiftPattern } from './worked-time.ts'
 ...
 * why the count is checked before the pattern — a driver with a running shift AND a closed one is
 * a double already, and asking `workedTime` about the live row would only return `pending`.
 */
