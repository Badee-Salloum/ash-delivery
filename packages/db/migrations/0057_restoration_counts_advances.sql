-- 0057 - الترميم counts an outstanding «سلفة» as office capital (owner decision 17)
--
-- Without this file, 0056 is actively dangerous. An advance empties the box while the money is
-- still company property, so a guard that knows only about الذمم reads the difference as a capital
-- shortfall and posts a «شحن» from صندوق الشركة every night — then sweeps it back when the advance
-- is repaid. The company fund would finance every advance, invisibly.
--
-- The owner's arithmetic, with one term added:
--
...
--
-- Office capital therefore falls at the conversion and not one moment earlier, which is what
-- «يجب أن ترد كاملة» means once it is written in double entry rather than in a policy document.
--
-- ── HOW THE GUARD BELOW WAS PRODUCED ─────────────────────────────────────────────────────────
-- 0053 CANNOT BE EDITED: `migrate.ts` checksums every applied migration and refuses a changed one.
-- So the v3 function is re-emitted here under a new name. It was NOT retyped. It was extracted
-- verbatim (`sed -n '51,382p' 0053_ledger_backed_restoration.sql`) and changed by exactly seven
-- named hunks, proven by `diff -u`: 40 lines of 332.
--
--   1. the function name
--   2. the schema version it answers for, '3' -> '4'
--   3. BOTH leg key arrays gain 'advances' — the require-list and the exact-list. Missing the
--      second would reject every plan the API emits
--   4. the money-shape regex applies to the new term
--   5. the position identity becomes officeBalance + receivables + advances
--   6. a NEW, INDEPENDENT disjunct derives the advance sum from the ledger. The driver-receivable
--      subquery beside it is BYTE-IDENTICAL — a test pins that
--   7. the postcondition splits advances out of receivables
--
-- Untouched, and it matters that this can be said precisely: the actor check, the advisory lock,
-- the opening-balance shape, the journal-id cardinality, the journal-line matching, the opening
-- snapshot reversal, and the net computation. Four whole blocks unchanged.
--
-- WHY v4 AND NOT "fold advances into receivables". `restorations.plan` is immutable and IS the
-- audit artifact. A v3 row would silently mean «driver receivables» before this release and
-- «driver receivables + advances» after, with nothing to tell them apart — and the Treasury screen
-- renders `leg.receivables` as «الذمم», so a manager would read an advance as driver debt.
--
-- The v3 function and its trigger are deliberately left in place, for the same reason 0053 left v2
-- in place: they are the exact rule already-stored history was judged by.
...
CREATE TRIGGER restorations_10_v4_guard
  BEFORE INSERT ON restorations
  FOR EACH ROW
  WHEN ((NEW.plan->>'schemaVersion') = '4')
  EXECUTE FUNCTION guard_ledger_restoration_insert_v4();

-- The dispatcher, extracted verbatim from 0053:16-37 and changed by four named edits: accept '4',
-- treat v4's evidence source like v3's, and refuse a v3 plan while any advance is outstanding.
CREATE OR REPLACE FUNCTION guard_restoration_schema_insert() RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public, pg_temp
AS $$
BEGIN
  IF jsonb_typeof(NEW.plan) IS DISTINCT FROM 'object'
     OR COALESCE(NEW.plan->>'schemaVersion', '') NOT IN ('2', '3', '4')
  THEN
  ...
  IF (NEW.plan->>'schemaVersion' = '2' AND NEW.cash_count_id IS NULL)
     OR (NEW.plan->>'schemaVersion' IN ('3', '4') AND NEW.cash_count_id IS NOT NULL)
  THEN
  ...
  -- A v3 plan has no `advances` term, so it would read an outstanding سلفة as a capital shortfall
  -- and «شحن» real money out of صندوق الشركة to refill a box that is not actually short — every
  -- night, and again in reverse when the advance is repaid. The only way to emit one after this
  -- migration is an API rolled back past the advances release, so refuse it by name: الترميم then
  -- STOPS, loudly and recoverably, instead of quietly paying for the same advance twice.
  --
  -- Deliberately NOT security definer: this runs as app_user, which already reads both tables on
  -- every ledger balance.
  IF NEW.plan->>'schemaVersion' = '3'
     AND EXISTS (
       SELECT 1
         FROM public.funds f
         JOIN public.journal_lines jl ON jl.fund_id = f.id
        WHERE f.branch_id = NEW.branch_id
          AND f.type::text IN ('advance_receivable_cash', 'advance_receivable_wallet')
        GROUP BY f.id
       HAVING sum(CASE jl.side WHEN 'D' THEN jl.amount_minor ELSE -jl.amount_minor END) <> 0
     )
  THEN
    RAISE EXCEPTION 'schema-v3 restoration cannot ignore an outstanding advance'
      USING ERRCODE = '23514', CONSTRAINT = 'restorations_plan_version_guard';
  END IF;
...
COMMENT ON FUNCTION guard_ledger_restoration_insert_v4() IS
  ...
  'outstanding advances, targets, journals, net, and postcondition. Byte-identical to '
  'guard_ledger_restoration_insert() apart from seven named hunks adding the advances term.';

COMMENT ON FUNCTION guard_ledger_restoration_insert() IS
  'Schema-v3 guard, retained for the history it judged. Since 0057 a v3 plan is refused outright '
  'while any advance is outstanding, because it has no advances term and would read one as a '
  'capital shortfall.';
