...
describe('320px RTL close screen guards', () => {
  ...
  it('keeps one compact save failure out of the missing-items list and gates submission', () => {
    ...
    expect(shift).toContain('t.shift.draftMergeConflictTitle')
    expect(shift).toContain('t.shift.usePhoneDraft')
    expect(shift).toContain('t.shift.useServerDraft')
    expect(shift).toContain("draftSaveNotice === 'conflict'")
  })
})
