...
| D-16 | **منظومة العرض الموحدة:** المنتج الظاهر هو **ASH Delivery** و**ASH GROUP** مرجع شركة ثانوي. تعتمد الواجهة العربية RTL والمصطلحات التشغيلية الموحدة، وتعرض الوقت عبر دمشق، وتستخدم رموز التصميم الدلالية وحوارات/حقولاً متاحة. يملك تطبيق السائق أيضاً خيارات Auto/Light/Dark. هذا قرار عرض وتجربة فقط؛ لا يغيّر الصلاحيات أو الحسابات أو الـAPI أو قاعدة البيانات. | 2026-09-19 |
...
| A-33 | **Completed-shift history means `approved` and `week_locked`; `cancelled` is shown separately.** ~~The default view is seven dates and a custom query is bounded to 31 dates.~~ **Amended 2026-09-17 (P2):** the default is the shared filter's «الكل منذ البدء»; one request covers at most 31 business dates for the branch, or 400 once a driver or vehicle narrows it (`GET /shifts` refuses more with 422 `range_too_large`). A longer selection shows its most recent days with a note naming them, never an empty page. | A cancelled shift is ended operationally but has no completed financial settlement. A bounded date-scoped read reuses the existing audited branch API without creating an unbounded browser fan-out. | `cheap` — history filter/range UI |
...
| A-37 | **«الكل منذ البدء» starts at the go-live business date, else the branch's first ledger activity, else today.** `GET /dashboard/meta` returns that `epoch` with the server's «today»; a go-live date set in the future never inverts the range. The chosen period is remembered per user (URL first, then `localStorage`), and the default is «الكل». | The owner asked for the cumulative view «منذ بدء العمل» by default but named no start. The go-live setting is the recorded start of real operations; the first activity is the honest fallback for a database without it. «Today» comes from the server because the session's business date goes stale after 04:00. | `cheap` — one resolver in `packages/domain/src/time/range.ts` |
| A-38 | **«هذا الشهر» / «الشهر الماضي» are calendar months of business dates** (a business day starts at 04:00 Asia/Damascus), and weeks start on **Sunday** (BR7). Trend bars bucket by day up to 62 days, by week up to 26 weeks, otherwise by month. | The owner's reference app (`Ash group v2`) was copied for its look only: it resolves dates in the browser's zone and starts weeks on Saturday, which would disagree with the Sunday close. | `cheap` |
| A-39 | **Fixed-asset book value is time-based over 36 months; period 1 is the purchase month.** Existing vehicles catch up every scheduled month through the current month. Disposal or sale before month 36 is deferred, and one asset may link to at most one vehicle. | These defaults were left open after the owner fixed the transfer behaviour. They make the schedule deterministic and keep a vehicle card from aggregating unrelated purchases. | `moderate` — disposal needs a new event/recipe |
| A-40 | **Historical company items are booked today and retain their real date in a separate column.** Profit counts them on the posting business date; lists show both posting and real dates. | Locked financial weeks cannot be rewritten safely, while hiding the real purchase/payment date would make the register misleading. | not reversible — period immutability |
| A-41 | **A company USD command defaults to today's system FX rate, may be overridden before posting, and freezes the chosen rate on the journal and command.** Balances may be displayed at today's rate, but historical profit converts each entry at its own frozen rate. | `fx_days` is mutable and therefore cannot reconstruct what a historical decision used. | `moderate` |
| A-42 | **Changing a recurring occurrence's amount at payment requires a written reason.** A skipped occurrence also requires a reason; neither action edits the template or another due date. | The template is a reminder, not an authority to move money, so the human decision and any deviation must remain legible. | `cheap` |
| A-43 | **Finance ranges use business dates:** «الكل منذ البدء» starts at go-live (first ledger activity fallback), and «هذا الشهر» is the calendar month of 04:00-based business dates. | One rule keeps dashboards, company movements, recurring due dates and vehicle history reconcilable. | `cheap` |
| A-44 | **Branch managers never receive asset purchase prices, instalments, outstanding company debts, depreciation schedules or book values.** Company debts never affect branch profit or treasury figures. | `company_fund.manage` is organisation-wide and owner-limited; exposing the fields in a branch payload would defeat server-side RBAC even if CSS hid them. | `structural` — wire/RBAC boundary |
| A-45 | **The ordinary recurring-expense due screen shows 92 days back and 7 days ahead.** Older unresolved occurrences are counted and disclosed, not silently dropped; an explicit due range is capped at 366 days. | Three months keeps quarterly misses visible without allowing a daily template to create an unbounded response. The separate older count makes the bound honest. | `cheap` — constants in `expenses/recurrence.ts` |
| A-46 | **A recurring payment defaults to its due date while that financial week is open; otherwise it posts today.** A manager-chosen date must be between the due date and today, inclusive, and BR7 still rejects a sealed week. | This places a timely cost in its intended period without reopening history. Once the period is sealed, today is the only honest new posting date. | `moderate` — changes the period carrying the expense |
| A-47 | **Deactivation takes effect on today's due date and every later date; earlier unpaid dates remain due.** A schedule edit may change future promises but may never make an already paid/skipped date invalid. | Stopping a contract should not erase an old debt or rewrite a human decision. The database guard is the final authority for resolved history. | `moderate` — different cut-off semantics would change which dates remain payable |
| A-48 | **A self-registered driver is active immediately and receives the same operational access as an admin-created driver.** Public branch names/IDs may be listed. There is no approval, invitation, CAPTCHA, SMS, email or extended profile; the only anti-abuse control is three schema-valid submissions per normalized address in a rolling hour. | Owner-approved scope for the driver app. The account, linked driver and eight-hour session are one atomic write, so a successful response can enter vehicle selection immediately. | `cheap` operationally via `DRIVER_SELF_REGISTRATION_ENABLED=false`; changing activation/verification needs a new product flow |
| A-49 | **Treasury movement direction is the signed sum of the cash-box and wallet effects:** positive is incoming, negative is outgoing, and zero is internal only when at least one office balance actually changed. The event's registration clock is `journal_entries.created_at` rendered in Asia/Damascus through seconds; `business_date` remains a separate accounting day. | The owner explicitly asked to distinguish the request/accounting day from the time the journal was actually entered, including after-midnight movements. A same-fund debit/credit that nets both office effects to zero is not a movement and must not be mislabeled as an internal transfer. | `cheap` — one read-model predicate and presentation rule |
| A-50 | **User-facing instants use the shared Asia/Damascus client formatter, not the browser locale or device time zone.** `وقت التسجيل` is the entered-journal instant and `يوم العمل` is its separate accounting date. | A manager may review a movement after midnight or from another time zone; browser-local formatting would make the operational record disagree with the ledger's Damascus day. | `cheap` — presentation helper and tests only |
| A-51 | **New presentation code uses semantic design tokens and shared accessible controls.** Raw color additions, browser confirms, and unlabelled/unlinked form controls are treated as regressions; existing behavior is migrated in safe screen batches. | The common contract makes dark mode, RTL, contrast, keyboard behavior, and failure states consistent without silently changing operational behavior. | `cheap` per screen; no finance/API/data migration |
