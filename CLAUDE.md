...
| 27 | **The recorded GPS path is linked to each order by printed time** (2026-09-21). Live tracking (SRS K) was already built; this reads the stored trail. `sliceTrailByOrders` (`packages/domain/src/gps/segment.ts`, pure) splits a shift's capture-ordered pings into a **path segment per order**, START-OWNED: an order owns the trail from its own printed minute to the next timed order's; the last runs to `submitted_at`; pings before the first order or at/after submit are their own buckets; an order whose printed minute is unreadable gets **no** segment. This is **best-effort by printed time — never proof** a position belongs to an order, because an order's time is a minute-precision screenshot claim and is often null. Served by `GET /shifts/:id/gps/path` (`gps.view`) and shown by one reusable `ShiftPathMap` in the review overlay, the vehicle history and a standalone screen. A tracked live shift that has sent nothing for 10 minutes is surfaced as "not reporting" on the live map, because a stopped phone tracker vanishes rather than turning red. **Hardware tracker infrastructure only** (no device exists yet): `tracker_devices` (0080) registers a bike-mounted unit by IMEI (hash-only secret, one active device per bike) under `fleet.manage`, and `POST /tracker/ingest` is the gateway seam — public in the RBAC sense (a unit has no session), guarded by `TRACKER_GATEWAY_TOKEN`, **404 until `TRACKER_INGEST_ENABLED`**, resolving a device's bike to its live shift and forcing `source='tracker'`. Off-shift fixes are dropped. Phone-side background survival is hardened in the Android shell (`android/native`), most of it from an adversarial audit (2026-09-21): a platform-`LocationManager` fallback when Play Services is absent (Huawei/EMUI), a watchdog that re-arms the location request when fixes stop, a wake-lock spanning each upload, a guarded `startForeground` so a permission-less restart on Android 14+ stops cleanly instead of crash-looping, a `BootReceiver` (gated on `ACCESS_BACKGROUND_LOCATION`) that restarts tracking after a reboot, a one-time battery-optimisation exemption and a one-time OEM-autostart deep-link against power-killers, `stopWithTask="false"` plus an `onTaskRemoved` restart so swiping the app off the recents list does not stop tracking, and — because a 12h double shift outlives the 8h session — the session cookie now slides on activity (server) and is written back into the WebView jar by the tracker (native) so uploads never silently 401. Only a deliberate force-stop (Settings / OEM security-centre) is beyond the app; the server "not reporting" alert surfaces any stopped tracker, and the GT06 hardware gateway (deferred to procurement) removes the phone dependency entirely. **Per-order path length (2026-09-23):** `pathLengthMetres` (`packages/domain/src/gps/distance.ts`, pure — the haversine now lives in `packages/domain/src/geo/haversine.ts`, shared with attendance) sums the great-circle hops of each segment's ping slice; the `/gps/path` response carries whole-metre `distanceMetres` on every segment/`beforeFirst`/`afterClose` and a `totalDistanceMetres`. Shown beside each order — in the review orders table (a distance column + a shift total), the close-workspace order cards/rows, and every `ShiftPathMap` side list (per order + whole-trail total) — best-effort by the recorded trail, never an odometer reading. |
| 26 | **Human-confirmed asset installment plans** (2026-09-20, explicitly amends decision 22). A financed fixed asset may have one active plan over its existing linked payable: a fixed amount, first due date, default source, and one recurrence (`monthly_first`, weekly weekday, or every N days). It is a reminder, never a cron or authority to move money. A company-fund manager explicitly pays or skips each due date; a payment writes the normal immutable debt-payment event and guarded journal, capped at the remaining balance for the final installment, while a skip and deactivation require written reasons. The default source is editable at payment confirmation. Deactivating a plan stops today and later dues but leaves earlier unresolved dues payable/skippable and preserves every resolved fact. A plan may be created atomically with a new financed purchase or later for an unpaid existing financed asset. |
